...
export const assignTeacher = async (req, res) => {
  try {
    ...
    if (role !== "campus-admin") {
      return res
        .status(403)
        .json({ error: "Only campus-admins can assign teachers" });
    }

    const campus = await Campus.findOne({ campusAdmin: _id }).select(
      "_id name"
    );
    if (!campus) {
      return res
    ...
    if (
      role === "campus-admin" &&
      teacher.createdBy?.toString() !== _id.toString()
    ) {
      return res
        .status(400)
...
export const enrollStudent = async (req, res) => {
  try {
    ...
    if (campus._id.toString() !== campusId.toString()) {
      return res
        .status(403)
        .json({ error: `You can only enroll students in your own campus: ${campus.name}` });
    }
